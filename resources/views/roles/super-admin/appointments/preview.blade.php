        ...
        <div class="d-flex">
            ...
            @if (!$appointment->isCompleted)
                ...
                @if ($appointment->isRequested)
                    ...
                    <a href={{ route('super_admin.dashboard.appointments.edit', $appointment->id) }} type="button"
                        class="btn btn-primary mx-1">
                        <span>Edit</span>
                    </a>
                    {{-- Requested appointments can be deleted --}}
                    <form action="{{ route('super_admin.dashboard.appointments.delete', $appointment->id) }}"
                ...
                @elseif (!$appointment->isDeclined())
                    {{-- Received appointments can be declined once only --}}
                    <form action="{{ route('super_admin.dashboard.appointments.decline', $appointment->id) }}"
                        method="POST" class="ml-1">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-danger btn-dim">
        ...
        <div class="card-inner card-inner-xl">
            <div class="entry">
                ...
                <div class="row py-3">
                    ...
                    <div class="col-sm-6">
                        <h6>Admin Name</h6>
                        <p>{{ $appointment->admin->user->name }}</p>
                    </div>
                </div>
                ...
                @if ($appointment->isPending() && $appointment->meeting_id && $appointment->meeting_platform == "Cognimeet")
                    ...
                    <a href="{{ $appointment_meta['start_url'] }}" target="blank" role="button" class="btn btn-primary mt-3">
                        <span>Start Session</span>
                    </a>

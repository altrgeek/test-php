        ...
        <div class="d-flex">
            ...
            @if (!$appointment->isCompleted)
                ...
                @if ($appointment->isRequested)
                    ...
                    <a href={{ route('admin.dashboard.appointments.provider.edit', $appointment->id) }} type="button"
                        class="btn btn-primary mx-1">
                        <span>Edit</span>
                    </a>
                    {{-- Requested appointments can be deleted --}}
                    <form action="{{ route('admin.dashboard.appointments.provider.delete', $appointment->id) }}"
                ...
                @else
                    {{-- Can be reviewed if not already --}}
                    @if ($appointment->isBooked())
                        <a
                            href={{ route('admin.dashboard.appointments.provider.reviewForm', $appointment->id) }}
                            type="button"
                    ...
                    @if (!$appointment->isDeclined())
                        ...
                        <form
                            action="{{ route('admin.dashboard.appointments.provider.decline', $appointment->id) }}"
                            method="POST"
                            class="ml-1"
                        >
                            @csrf
